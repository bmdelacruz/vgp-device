...
impl FD {
    pub fn new(flags: OFlag) -> Result<Self, nix::Error> {
        "/dev/uinput"
            .with_nix_path(|p| {
...
impl UInputFD {
    fn new() -> Result<Self, Error> {
        let fd = FD::new(OFlag::O_RDWR | OFlag::O_NONBLOCK).map_with_vgp_error()?;
    ...
    fn drop(&mut self) {
        unsafe {
            if let Err(e) = ui_dev_destroy(self.fd.0) {
                log::error!("Failed to destroy uinput device: {:?}", e);
            }
        }
    }
}

pub fn create_uinput_device() -> Result<(UInputFDReader, UInputFDWriter), Error> {
    let ufd = UInputFD::new()?;
    let ufd = Arc::new(ufd);
    let ufd_clone = Arc::clone(&ufd);
    ...
    Ok((
        ...
        UInputFDWriter { ufd: ufd_clone },
    ))
}

pub struct UInputFDReader {
    ufd: Arc<UInputFD>,
...
impl UInputFDReader {
    ...
    pub fn read(&mut self) -> Result<Output, Error> {
        ...
        let n = unsafe {
            let n = libc::read(self.ufd.fd.0, input_event.c_ptr(), InputEvent::size());
                ...
                if n != InputEvent::size() {
                    ...
                    if input_event.r#type == EV_UINPUT as u16 {
                        if input_event.code == UI_FF_UPLOAD as u16 {
                            ...
                            unsafe {
                                ui_begin_ff_upload(self.ufd.fd.0, force_feedback_upload.mut_ptr())
                                    .map_with_vgp_error()?;
                            }
                            ...
                            unsafe {
                                ui_end_ff_upload(self.ufd.fd.0, force_feedback_upload.mut_ptr())
                                    .map_with_vgp_error()?;
                            }
                            ...
                            unsafe {
                                ui_begin_ff_erase(self.ufd.fd.0, force_feedback_erase.mut_ptr())
                                    .map_with_vgp_error()?;
                            }
                            ...
                            unsafe {
                                ui_end_ff_erase(self.ufd.fd.0, force_feedback_erase.mut_ptr())
                                    .map_with_vgp_error()?;
                            }
...
pub struct UInputFDWriter {
    ufd: Arc<UInputFD>,
}

impl UInputFDWriter {
    pub fn write(&mut self, input: Input) -> Result<(), Error> {
        ...
        match input {
            Input::Press(button) => {
                ...
                unsafe {
                    let n =
                        libc::write(self.ufd.fd.0, press_input_event.c_ptr(), InputEvent::size());
        ...
        unsafe {
            let n = libc::write(self.ufd.fd.0, press_input_event.c_ptr(), InputEvent::size());
            Errno::result(n).map_with_vgp_error()?;
        }
